...
If the column separation or the decimal separation is not the one expected by the function you choose, you can specify the correct one inside the function with `sep = ' '` and `dec = '.'`, respectively.
...
```{.r}
source("https://karengoncalves.github.io/Programming_classes/r/exampleData/script_class1.R")
```
